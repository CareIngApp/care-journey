...
// A single `cond` (or null) tailors the condition card; multiple conditions are
// expanded into one card each by buildPlaybook, so every condition links to its own charity.
function resolveCard(actionId, phase, idx, cond, triage, suffix = '') {
  const base = ACTIONS[actionId];
  if (!base) return null;
  const card = {
    uid: actionId + '_' + idx + suffix,
    actionId, phase,
    ...base,
  ...
  // Condition tailoring: point this card straight at the chosen condition's charity.
  if (base.flags.includes('condition') && cond) {
    card.source = cond.source;
    card.helpline = HELPLINES[cond.helpline] || card.helpline;
    card.helplineRef = cond.helpline;
    card.title = 'Work through the ' + cond.label + ' to-do list';
    card.who = cond.org + ' has a newly-diagnosed guide written for ' + cond.label.toLowerCase() + '.';
  }
...
export function buildPlaybook(triggerId, conditionIds, triage) {
  ...
  // 1. Resolve cards with their default phase. A condition card is expanded into
  //    one card per chosen condition, so each points at its own charity to-do list.
  let cards = spec.flatMap(([aid, phase], i) => {
    const base = ACTIONS[aid];
    if (base && base.flags.includes('condition') && conds.length) {
      return conds.map(c => resolveCard(aid, phase, i, c, triage, '_' + c.id));
    }
    return [resolveCard(aid, phase, i, conds[0] || null, triage)];
  }).filter(Boolean);
